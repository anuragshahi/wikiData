    ...
    void Start () {
        ...
        for (var i = 0; i < pointList.Count; i++)
        {
            ...
            GameObject dataPoint = Instantiate(
                    PointPrefab, 
                    new Vector3(x*2, y*4, z*2)* plotScale, 
                    Quaternion.identity);
            ...
            Debug.Log("dataPoint val : " + dataPointName);
            Debug.Log("revert val : " + pointList[i][revert]);
            // Gets material color and sets it to a new RGB color we define
            if (pointList[i][revert].Equals("False"))
            dataPoint.GetComponent<Renderer>().material.color = Color.red;
            else
            dataPoint.GetComponent<Renderer>().material.color = Color.green;

            // new Color(x,y,z, a:1.0f);
        }       
